export const projects = [
  ...
  },
  {
    ...
    fullDescription: 'dustpeakclub은 Spotify 음악 데이터를 기반으로 자신만의 플레이리스트와 앨범 리스트를 만들고 공유하는 음악 큐레이션 커뮤니티입니다.\n\n' +
      '사용자는 곡과 앨범을 검색해 리스트를 구성하고, 공개된 콘텐츠를 최신순·인기순으로 탐색하며 좋아요, 북마크, 댓글, 팔로우로 서로의 음악 취향을 발견할 수 있습니다.\n\n' +
      'Next.js App Router를 중심으로 인증, API, 관계형 데이터 모델, 통합 피드, 커서 페이지네이션과 검색 구조를 설계했으며, 기획부터 UI·백엔드·DB·배포까지 전 과정을 단독으로 구현했습니다.',
    period: '2026.02 - 2026.07',
    team: '개인프로젝트',
    features: [
      'Supabase OAuth 로그인, 온보딩 및 서버·클라이언트 세션 관리',
      'Spotify Web API 기반 곡·앨범 검색과 플레이리스트·앨범 리스트 CRUD',
      '공개 범위, 태그, 중복 방지, 드래그 기반 음악 순서 관리',
      '최신순·인기순 정렬, 타입 필터와 커서 기반 무한 스크롤',
      '좋아요, 북마크, 댓글, 공유 및 조회수 집계',
      '콘텐츠·태그·음악·사용자를 아우르는 통합 검색과 Search Alias 관리',
      '프로필, 팔로우, 마이페이지 및 사용자 활동 대시보드',
      '동적 metadata, sitemap, robots와 반응형 UI 구성',
    ],
    challenges: [
      'Next.js App Router 환경에서 서버 컴포넌트와 클라이언트 컴포넌트의 역할 분리',
      'Playlist와 AlbumList의 공통 로직을 통합하되 타입별 데이터 구조와 권한 처리는 분리',
    ...
    outcomes: [
      '인증부터 콘텐츠 생성·탐색·상호작용까지 이어지는 서비스 전 과정을 단독 구축',
      '서로 다른 음악 리스트 도메인을 공통 피드와 UI로 확장할 수 있는 구조 설계',
      '관계형 데이터 모델과 복합 제약 조건을 통해 소셜 기능의 데이터 정합성 확보',
      '외부 음악 API, 검색 보정, 페이지네이션과 SEO를 포함한 운영 가능한 서비스 배포',
    ]
  }
];
