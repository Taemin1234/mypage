export const projects = [
  ...
  },
  {
    ...
    links: {
      ...
      github: 'https://github.com/Taemin1234/taest-mag',
  ...
  },
  {
    ...
    features: [
      ...
      '로그인 이후 닉네임을 설정하는 온보딩 플로우',
      'Spotify Web API를 활용한 곡·앨범 검색 기능',
      '플레이리스트 및 앨범리스트 생성·수정·삭제 기능',
      '공개/비공개 설정, 태그 입력, 음악 항목 중복 방지 및 순서 변경 기능',
      '최신순·좋아요순 정렬과 타입 필터를 지원하는 리스트 탐색 화면',
      'IntersectionObserver 기반 무한 스크롤 및 커서 기반 페이지네이션',
      '좋아요, 북마크, 댓글, 공유 기능을 포함한 리스트 상세 페이지',
      '게시글 제목·내용, 태그, 음악 정보, 사용자 닉네임 기반 통합 검색',
      '음악 검색 정확도 개선을 위한 Search Alias 관리 구조',
      '프로필 페이지, 팔로우/팔로워 목록, 사용자별 공개 리스트 제공',
      '마이페이지에서 내가 만든 리스트, 좋아요한 리스트, 북마크한 리스트 조회',
      '조회수, 좋아요, 공개 비율, 주간 지표 등을 확인할 수 있는 사용자 대시보드',
      'Next.js metadata, sitemap, robots 설정을 통한 SEO 기반 구성',
      '모바일·태블릿·데스크톱을 고려한 반응형 UI'
    ],
    challenges: [
      'Next.js App Router 환경에서 서버 컴포넌트와 클라이언트 컴포넌트의 역할 분리',
      'Supabase Auth 사용자와 서비스 DB User 모델을 동기화하는 구조 구현',
      'Playlist와 AlbumList처럼 유사하지만 다른 도메인을 공통 UI와 API 흐름으로 다루는 구조 설계',
      'Prisma 기반으로 좋아요, 북마크, 댓글, 팔로우, 조회수 등 복합 관계 데이터 모델링',
      'ListFeed 모델을 활용해 플레이리스트와 앨범리스트를 하나의 피드로 통합하는 방식 설계',
      '무한 스크롤에서 중복 요청, 중복 데이터, 탭별 캐싱 문제를 처리',
      '검색어, 태그, 음악명, 아티스트명, 사용자 닉네임을 분리해 검색하는 통합 검색 구조 구현',
      '공개/비공개 리스트 접근 권한과 작성자 권한 검증을 API 단에서 처리'
    ],
    outcomes: [
      'Next.js App Router 기반 풀스택 애플리케이션 설계 및 구현 경험 확보',
      'Supabase Auth와 Prisma DB를 연동한 인증/사용자 관리 구조 이해도 향상',
      'PostgreSQL 관계형 모델링을 통해 커뮤니티 서비스의 데이터 구조를 설계',
      'Spotify Web API 연동, 토큰 캐싱, 외부 API 검색 흐름 구현 경험 확보',
      '커서 기반 페이지네이션, 무한 스크롤, 클라이언트 캐싱을 활용한 탐색 UX 구현',
      '좋아요, 북마크, 댓글, 팔로우, 대시보드 등 실제 서비스형 소셜 기능 구현 경험',
      '검색 별칭, 통합 검색, SEO 설정 등 콘텐츠 플랫폼에 필요한 확장 기능 구현',
      '기획부터 UI, 프론트엔드, 백엔드, DB 설계, 배포까지 1인 풀스택 프로젝트로 포트폴리오화 가능'
    ]
  }
];
